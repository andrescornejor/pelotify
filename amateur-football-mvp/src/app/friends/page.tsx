...
export default function FriendsPage() {
    ...
    const handleSendRequest = async (friendId: string) => {
        ...
        try {
            ...
            setSearchResults(prev => prev.map(p => p.id === friendId ? { ...p, relationshipStatus: 'pending_sent' } : p));
        } catch (err: any) {
        ...
        <div className="flex flex-col min-h-[90vh] bg-background p-4 sm:p-6 lg:px-10 lg:pt-4 xl:px-14 2xl:px-16 max-w-screen-2xl mx-auto space-y-8 relative overflow-hidden">
            ...
            <div className="flex flex-col items-center text-center gap-4 relative z-10 px-4 mb-2">
                <div className="flex items-center gap-4 justify-center">
                    ...
                    <span className="text-[11px] font-black text-primary uppercase tracking-[0.5em] italic">Comunidad & Alianzas</span>
                    <div className="h-[1px] w-12 bg-primary/40" />
                </div>
                ...
                <div className="flex p-2 bg-foreground/[0.02] backdrop-blur-3xl rounded-[2.5rem] border border-foreground/[0.05] relative shadow-2xl">
                    <button
                        ...
                        className={cn(
                            ...
                            activeTab === 'friends' ? 'text-background' : 'text-foreground/40 hover:text-foreground/70'
                        )}
                    >
                        {activeTab === 'friends' && (
                            <motion.div
                    ...
                    <button
                        ...
                        className={cn(
                            "flex-1 flex items-center justify-center gap-3 py-4 text-[11px] font-black uppercase tracking-[0.3em] rounded-[1.8rem] transition-all relative",
                            activeTab === 'requests' ? 'text-background' : 'text-foreground/40 hover:text-foreground/70'
                        )}
                    >
                        {activeTab === 'requests' && (
                            <motion.div
                    ...
                    <button
                        ...
                        className={cn(
                            "flex-1 flex items-center justify-center gap-3 py-4 text-[11px] font-black uppercase tracking-[0.3em] rounded-[1.8rem] transition-all relative",
                            activeTab === 'search' ? 'text-background' : 'text-foreground/40 hover:text-foreground/70'
                        )}
                    >
                        {activeTab === 'search' && (
                            <motion.div
                            ...
                            {friends.length === 0 ? (
                                ...
                                >
                                    ...
                                    <div className="max-w-sm space-y-4">
                                        <h3 className="text-3xl font-black text-foreground italic uppercase tracking-tighter">Sin Amigos Aún</h3>
                                        <p className="text-[10px] text-foreground/60 font-black uppercase tracking-[0.3em] leading-relaxed">Todavía no tenés amigos agregados. ¡Buscá gente y sumá pibes!</p>
                                    </div>
                                    <button
                                        onClick={() => setActiveTab('search')}
                                ...
                                friends.map((f, i) => (
                                    ...
                                    >
                                        ...
                                        <Link href={`/profile?id=${f.profiles?.id}`} className="flex items-center gap-6 group/link">
                                            <div className="w-20 h-20 rounded-[2rem] bg-surface-elevated border-2 border-foreground/10 shrink-0 overflow-hidden relative group-hover:border-primary/40 group-hover:scale-105 transition-all duration-500 shadow-2xl">
                                                {f.profiles?.avatar_url ? (
                                                    <img src={f.profiles.avatar_url} alt="" className="w-full h-full object-cover grayscale-[0.3] group-hover:grayscale-0 transition-all" />
                                                ) : (
                                            ...
                                            <div className="flex-1 min-w-0 space-y-1">
                                                <p className="font-black text-2xl text-foreground italic uppercase truncate tracking-tighter group-hover/link:text-primary transition-colors">{f.profiles?.name}</p>
                                                <div className="flex items-center gap-2">
                                                    <div className="w-2 h-2 rounded-full bg-primary animate-pulse" />
                                                    <span className="text-[9px] font-black uppercase tracking-widest text-foreground/60">{f.profiles?.position || 'Jugador Polivalente'}</span>
                                                </div>
                                            </div>
                                        </Link>
                                        ...
                                        <div className="flex items-center justify-between pt-4 border-t border-foreground/5">
                                            <div className="flex items-center gap-6">
                                                <div className="flex flex-col">
                                                    <span className="text-[8px] font-black uppercase tracking-widest text-foreground/70">EXPERIENCIA</span>
                                                    <div className="flex items-center gap-2">
                                                        <Trophy className="w-3.5 h-3.5 text-accent" />
                                                        <span className="text-sm font-black text-foreground italic tracking-tighter">{f.profiles?.elo || 0}</span>
                                                    </div>
                                                </div>
                                                <div className="flex flex-col">
                                                    <span className="text-[8px] font-black uppercase tracking-widest text-foreground/70">RANGO</span>
                                                    <span className="text-[10px] font-black text-primary uppercase italic tracking-widest">PREMIER</span>
                                                </div>
                                            </div>
                        ...
                        <div className="space-y-6">
                            <div className="flex items-center justify-between px-2">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex flex-col">
                                        <h3 className="text-2xl font-black text-foreground italic uppercase tracking-tighter">Peticiones de Amistad</h3>
                                        <span className="text-[10px] font-black text-foreground/40 uppercase tracking-widest">Aspirantes a tu red de contactos</span>
                                    </div>
                                </div>
                                <div className="px-5 py-2 bg-primary/5 rounded-full border border-primary/20 shadow-sm">
                                    <span className="text-[10px] font-black text-primary uppercase tracking-widest">{requests.length} PENDIENTES</span>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {requests.length === 0 ? (
                                    <div className="col-span-full text-center py-20 glass-premium rounded-[3rem] border-dashed border-2 border-foreground/5 opacity-40 bg-foreground/[0.01]">
                                        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-foreground/70 italic">No hay peticiones de alianza en espera</p>
                                    </div>
                                ) : (
                                    requests.map((r, i) => (
                                        <motion.div
                                        ...
                                        >
                                            <Link href={`/profile?id=${r.profiles?.id}`} className="flex-1 flex items-center gap-6 group/link">
                                                ...
                                                <div className="flex-1 min-w-0 space-y-1">
                                                    <p className="font-black text-lg text-foreground uppercase italic truncate tracking-tighter group-hover/link:text-primary transition-colors">{r.profiles?.name || 'Nuevo Aspirante'}</p>
                                                    <div className="flex items-center gap-2">
                                                        <div className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse" />
                                                        <p className="text-[10px] text-foreground/50 font-black uppercase tracking-widest">Convocatoria Activa</p>
                                                    </div>
                                                </div>
                                            </Link>
                                            <div className="flex gap-2">
                                                ...
                                                >
                                                    {actionLoading === r.id ? <Loader2 className="w-5 h-5 animate-spin" /> : <Check className="w-5 h-5" />}
                                                </button>
                                                <button
                    ...
                    <div className="space-y-12 px-4 w-full mx-auto">
                        <div className="relative group w-full max-w-7xl mx-auto">
                            ...
                            <div className="relative bg-background rounded-[2.5rem]">
                                <div className="absolute inset-y-0 left-6 flex items-center pointer-events-none">
                                    <Search className={cn("w-6 h-6 transition-colors duration-300", searchQuery.length > 0 ? "text-primary" : "text-foreground/30")} />
                                </div>
                                <input
                                    type="text"
                                ...
                                <div className="absolute inset-y-0 right-6 flex items-center">
                                    <div className={cn(
                                        ...
                                        isSearching ? "bg-primary text-background border-primary" : "bg-primary/5 text-primary border-primary/20"
                                    )}>
                                        <div className={cn("w-1.5 h-1.5 rounded-full bg-current", isSearching && "animate-pulse")} />
                        ...
                        {isSearching ? (
                            <div className="relative py-32 flex flex-col items-center justify-center overflow-hidden rounded-[3rem] border border-primary/10 bg-primary/[0.01]">
                                ...
                                <div className="relative z-10 space-y-8 flex flex-col items-center">
                                    ...
                                    <div className="flex flex-col items-center gap-3">
                                        <h3 className="text-xl font-black text-primary italic uppercase tracking-[0.4em] animate-pulse">Buscando Jugadores</h3>
                            ...
                            searchResults.length === 0 ? (
                                ...
                                >
                                    <p className="text-xl font-black uppercase tracking-[0.2em] text-foreground/60 italic">Sin señales detectadas</p>
