...
export async function createTeam(name: string, description: string, captainId: string, logoUrl?: string) {
    const { data, error } = await supabase
        .from('teams')
        .insert([
            {
...
export async function joinTeam(teamId: string, userId: string, role: 'captain' | 'member' = 'member') {
...
export async function updateTeam(teamId: string, updates: Partial<Team>) {
    ...
    if (!data || data.length === 0) {
        throw new Error('No se pudo actualizar el equipo. Verifica que seas el capitán y tengas permisos.');
    }
...
export async function respondToTeamInvitation(teamId: string, userId: string, action: 'accept' | 'decline') {
    if (action === 'decline') {
        const { error } = await supabase
            .from('team_members')
            .delete()
