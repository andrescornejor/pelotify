...
import { Layout as LayoutIcon, Save, Plus, Trash2, CheckCircle2, Info, User2, Users, MousePointer2, Settings2, ShieldCheck, X, Search, ChevronRight } from 'lucide-react';
...
export default function TeamTactics({ teamId, isCaptain, members }: TeamTacticsProps) {
  ...
  function assignPlayer(nodeId: string, playerId: string | undefined) {
    if (!isCaptain) return;
    setNodes(prev => prev.map(n => 
      n.id === nodeId ? { ...n, playerId } : n
    ));
    setSelectingNodeId(null);
  }

  const assignedPlayerIds = new Set(nodes.map(n => n.playerId).filter(Boolean));
  const bench = members.filter(m => m.status === 'confirmed' && !assignedPlayerIds.has(m.user_id));

  if (isLoading) return <div className="p-12 text-center text-foreground/40 animate-pulse uppercase font-black text-[10px] tracking-widest italic">Escaneando Pizarras...</div>;
      ...
      <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-8 px-2">
        <div className="space-y-2">
          <div className="flex items-center gap-4">
             ...
             <div>
                <h3 className="text-3xl font-black text-foreground italic uppercase tracking-tighter leading-none">Pizarra del Capitán</h3>
                <p className="text-[10px] font-black uppercase tracking-[0.3em] text-foreground/40 mt-1">Definí el ADN táctico del equipo</p>
             </div>
          </div>
        </div>
        ...
        {isCaptain && (
          <div className="flex flex-wrap items-center gap-4 bg-foreground/[0.03] p-2 rounded-[2.5rem] border border-foreground/5 backdrop-blur-3xl shadow-2xl">
              ...
              {(['F5', 'F7', 'F11'] as MatchType[]).map(type => (
                <button
                  ...
                  className={cn(
                    ...
                    matchType === type ? "bg-primary text-background shadow-lg shadow-primary/30" : "text-foreground/40 hover:text-foreground/70"
                  )}
                >
                  {type}
                </button>
            ...
            >
              {isSaving ? <LoaderIcon className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
              GUARDAR ADN
            </button>
          </div>
        )}
                    ...
                    >
                      <div className="flex flex-col items-center gap-3 -translate-x-1/2 -translate-y-1/2 group/node w-max">
                        <div className={cn(
                          ...
                          node.role === 'DEL' ? "bg-red-600 border-red-400" :
                          node.role === 'MED' ? "bg-blue-600 border-blue-400" :
                          node.role === 'DEF' ? "bg-zinc-800 border-zinc-500" : "bg-yellow-500 border-yellow-300"
                        )}>
                          {player?.avatar_url ? (
                            <img src={player.avatar_url} className="w-full h-full object-cover rounded-full" alt="" />
                          ) : (
                          ...
                          {isCaptain && node.playerId && (
                             <button 
                               onClick={(e) => { e.stopPropagation(); assignPlayer(node.id, undefined); }}
                        ...
                        <div className={cn(
                            ...
                            player ? "bg-background/90 border-white/10" : "bg-black/60 border-white/5 animate-pulse"
                        )}>
                           <span className={cn(
                ...
                >
                    <div className="flex items-center justify-between mb-8">
                        <div className="flex items-center gap-4">
                            ...
                            <h4 className="text-2xl font-black italic text-foreground uppercase tracking-tighter">Asignar al {nodes.find(n => n.id === selectingNodeId)?.role}</h4>
                        </div>
                        <button onClick={() => setSelectingNodeId(null)} className="w-12 h-12 bg-foreground/5 rounded-2xl flex items-center justify-center text-foreground/40 hover:text-foreground transition-all">
                            <X className="w-6 h-6" />
                        </button>
                    </div>
                            ...
                            bench.map(member => (
                                ...
                                >
                                    ...
                                    <div className="text-left">
                                        <p className="text-sm font-black italic uppercase tracking-tighter text-foreground group-hover:text-primary transition-colors">{member.profiles?.name}</p>
                                        <p className="text-[9px] font-black text-foreground/40 uppercase tracking-widest">{member.profiles?.position || 'SIN POSICION'}</p>
                                    </div>
                                </button>
                            ))
                        )}
        ...
        <div className="lg:col-span-4 space-y-8 order-1 lg:order-2">
            ...
            <div className="glass-premium p-8 rounded-[3rem] border border-foreground/5 bg-surface-elevated space-y-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h5 className="text-lg font-black text-foreground italic uppercase tracking-tighter">Estado del Plantel</h5>
                        <p className="text-[10px] font-black text-foreground/40 uppercase tracking-widest">Resumen táctico actual</p>
                    </div>
                </div>

                <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 bg-foreground/[0.02] rounded-2xl border border-foreground/5 text-center">
                        <span className="block text-2xl font-black text-primary italic leading-none">{nodes.filter(n => n.playerId).length}/{nodes.length}</span>
                        <span className="text-[8px] font-black text-foreground/40 uppercase tracking-widest mt-1">TITULARES</span>
                    </div>
                    <div className="p-4 bg-foreground/[0.02] rounded-2xl border border-foreground/5 text-center">
                        <span className="block text-2xl font-black text-foreground italic leading-none">{bench.length}</span>
                        <span className="text-[8px] font-black text-foreground/40 uppercase tracking-widest mt-1">EN EL BANCO</span>
                    </div>
                </div>
                ...
                <div className="p-5 bg-primary/5 border border-primary/20 rounded-2xl">
                    <p className="text-[9px] text-primary font-black uppercase tracking-widest leading-relaxed text-center italic">
                        "Elegí un rol en el campo (círculo parpadeante) y asignale un pibe del club para que aparezca su nombre."
                    </p>
                </div>
            </div>
            ...
            <div className="glass-premium p-8 rounded-[3rem] border border-foreground/5 bg-surface space-y-6">
                <div className="flex items-center justify-between">
                    <h5 className="text-xl font-black text-foreground italic uppercase tracking-tighter">Entrenando Afuera</h5>
                    <Users className="w-5 h-5 text-foreground/20" />
                </div>

                <div className="space-y-3">
                    {bench.length === 0 ? (
                        <div className="py-12 text-center text-foreground/20 uppercase font-black text-[9px] tracking-widest">No quedan suplentes</div>
                    ) : (
                        bench.slice(0, 5).map(member => (
                            <div key={member.user_id} className="flex items-center gap-4 p-3 bg-foreground/[0.02] rounded-2xl border border-foreground/5">
                                <div className="w-10 h-10 rounded-lg bg-surface-elevated flex items-center justify-center overflow-hidden">
                                     {member.profiles?.avatar_url ? (
                                        <img src={member.profiles.avatar_url} className="w-full h-full object-cover" alt="" />
                                     ) : (
                                        <span className="font-black text-primary/30 italic">{member.profiles?.name?.charAt(0)}</span>
                                     )}
                                </div>
                                <span className="text-[11px] font-black text-foreground/60 uppercase tracking-tighter italic">{member.profiles?.name}</span>
                                {isCaptain && <div className="ml-auto w-2 h-2 rounded-full bg-primary/30 animate-pulse" />}
                            </div>
                        ))
                    )}
                    {bench.length > 5 && (
                        <p className="text-[9px] text-center text-foreground/30 font-black uppercase tracking-widest">+ {bench.length - 5} MÁS EN EL BANCO</p>
                    )}
                </div>
            </div>
        </div>
