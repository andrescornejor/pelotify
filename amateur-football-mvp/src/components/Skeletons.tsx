...
export function GridListSkeleton({ count = 8 }: { count?: number }) {
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {[...Array(count)].map((_, i) => (
                <div key={i} className="glass-premium p-6 rounded-[2.5rem] flex items-center justify-between gap-4 border border-foreground/5 bg-surface relative overflow-hidden">
