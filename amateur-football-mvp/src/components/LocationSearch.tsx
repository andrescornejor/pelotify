  ...
  return (
    <div className="relative group w-full" ref={containerRef}>
      <div className="absolute inset-y-0 left-6 flex items-center pointer-events-none z-10">
        <Search
          className={`w-5 h-5 transition-colors ${isLoading ? 'text-primary animate-pulse' : 'text-foreground/20 group-focus-within:text-primary'}`}
        />
      </div>
      ...
      <AnimatePresence>
        {isOpen && results.length > 0 && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 5 }}
            className="absolute top-full left-0 right-0 z-[100] border border-foreground/10 rounded-3xl overflow-hidden mt-2 p-1 sm:p-2 shadow-2xl bg-background/98 sm:bg-background/95 sm:backdrop-blur-3xl"
          >
