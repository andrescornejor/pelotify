...
const FeatureBox = ({ title, desc, icon: Icon, delay }: any) => (
  ...
  >
    ...
    <div className="w-14 h-14 rounded-2xl bg-surface border border-white/5 flex items-center justify-center mb-6 shadow-inner group-hover:scale-110 group-hover:rotate-6 transition-transform">
...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-[#020205] text-white selection:bg-primary selection:text-black overflow-x-hidden font-sans scroll-smooth">
      ...
      <section className="py-44 px-6 lg:px-20 relative overflow-hidden">
         <div className="max-w-[1500px] mx-auto rounded-[4rem] bg-gradient-to-br from-[#0A0A0F] via-black to-[#0A0A0F] border border-primary/20 p-12 lg:p-32 relative overflow-hidden shadow-[0_50px_100px_rgba(0,0,0,0.6)]">
            <div className="absolute inset-0 bg-[#020205]/20 backdrop-blur-3xl -z-10" />
            <div className="absolute top-0 right-0 w-full h-full bg-[radial-gradient(circle_at_70%_15%,rgba(44,252,125,0.06),transparent)] pointer-events-none" />
            
            <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
               <div className="space-y-12">
                  <div className="space-y-8">
                     <div className="w-20 h-20 rounded-3xl bg-primary/10 border border-primary/30 flex items-center justify-center shadow-glow">
                        <MapPin className="w-10 h-10 text-primary" />
                     </div>
                     <h2 className="text-6xl md:text-[6rem] font-black font-kanit italic uppercase tracking-tighter leading-[0.85]">
                        PROFESIONALIZÁ <br /> <span className="text-primary">TU COMPLEJO.</span>
                     </h2>
                     <p className="text-sm md:text-2xl font-black uppercase tracking-[0.4em] text-white/35 max-w-xl leading-relaxed italic">
                        Digitalizamos tu gestión. Turnos asegurados, reportes financieros y ocupación máxima.
                     </p>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-12">
                     <div className="space-y-3 pl-8 border-l border-primary/40">
                        <DollarSign className="w-6 h-6 text-primary" />
                        <h5 className="text-[13px] font-black uppercase tracking-widest text-white">Ingresos Seguros</h5>
                        <p className="text-[10px] font-black uppercase tracking-widest text-white/25">Cobros de señas automáticos. Menos plantones.</p>
                     </div>
                     <div className="space-y-3 pl-8 border-l border-primary/40">
                        <BarChart3 className="w-6 h-6 text-primary" />
                        <h5 className="text-[13px] font-black uppercase tracking-widest text-white">Métricas Reales</h5>
                        <p className="text-[10px] font-black uppercase tracking-widest text-white/25">Panel estadístico de ocupación y caja diaria.</p>
                     </div>
                  </div>

                  <Link href="/canchas/login" className="inline-block pt-4">
                     <button className="px-16 h-20 bg-white text-black font-black uppercase text-[12px] tracking-[0.5em] rounded-[1.5rem] hover:bg-primary hover:scale-105 transition-all shadow-2xl flex items-center gap-4 group">
                        <span>SUMAR MI SEDE</span>
                        <ChevronRight className="w-5 h-5 group-hover:translate-x-2 transition-transform" />
                     </button>
                  </Link>
               </div>

               {/* Mockup / Power visualization */}
               <div className="hidden lg:flex justify-center perspective-1000 scale-110">
                  <div className="w-full max-w-md aspect-square rounded-[3.5rem] bg-[#0A0A0F] border border-white/5 flex flex-col justify-center p-14 relative shadow-[0_50px_100px_rgba(0,0,0,0.5)]">
                     ...
                     <div className="space-y-8 relative z-10">
                        <div className="p-8 rounded-[2rem] bg-white/[0.03] border border-white/10 space-y-3">
                           <p className="text-[10px] uppercase font-black tracking-[0.3em] text-white/40">Ocupación Mensual</p>
                           <p className="text-6xl font-black italic font-kanit text-primary leading-none tracking-tighter shadow-glow">97.4%</p>
                        </div>
                        <div className="space-y-4 px-2">
                           {[1, 2, 3].map(i => (
                              <div key={i} className="h-4 w-full rounded-full bg-white/[0.05] overflow-hidden">
                                 <motion.div 
                                    initial={{ width: 0 }}
      ...
      <section className="py-72 px-6 text-center overflow-hidden">
         <div className="max-w-6xl mx-auto space-y-20 relative z-10">
            <h2 className="text-7xl md:text-[14rem] font-black font-kanit italic uppercase tracking-tighter leading-[0.7] drop-shadow-[0_20px_60px_rgba(0,0,0,0.8)]">
               TU DESTINO <br /> ES EL <span className="text-primary italic animate-glow-pulse [text-shadow:0_0_80px_rgba(44,252,125,0.4)]">GREEN.</span>
            </h2>
            <Link href="/register">
