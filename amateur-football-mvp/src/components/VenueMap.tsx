...
export default function VenueMap({ location, lat, lng }: VenueMapProps) {
    ...
    const coords: [number, number] | null = (lat && lng) ? [lat, lng] : (venue ? [venue.lat, venue.lng] : null);
            ...
            <div className="flex items-center justify-between px-2">
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col">
                        <h2 className="text-2xl font-black text-foreground italic uppercase tracking-tighter">Ubicación de la cancha</h2>
                        <span className="text-[9px] font-black text-foreground/60 uppercase tracking-widest">{location}</span>
                    </div>
                </div>
                <a 
                    href={mapsUrl} 
